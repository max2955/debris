...
public class BodyEditorLoader {
	...
	public static class RigidBodyModel {
		...
		public final List<ShapeModel>  shapes = new ArrayList<ShapeModel>();
	}
	...
	public static class PolygonModel {
		...
		public float [] toFloatArray() {
			float [] f = new float[vertices.size()*2] ;
			int j = 0 ;
			for(int i=0;i<vertices.size();++i) {
				f[j] = vertices.get(i).x ;
				f[j+1] = vertices.get(i).y ;
				j += 2 ;
			}
			return f;
		}
	...
	private RigidBodyModel readRigidBody(JsonValue bodyElem) {
		...
		JsonValue shapeElem = bodyElem.getChild("shapes");

		for (; shapeElem != null ;shapeElem = shapeElem.next()){

			ShapeModel shape = new ShapeModel();
			rbModel.shapes.add(shape);

			JsonValue vertexElem = shapeElem.getChild("vertices");
			for (; vertexElem != null; vertexElem = vertexElem.next()) {
				float x = vertexElem.getFloat("x");
				float y = vertexElem.getFloat("y");
